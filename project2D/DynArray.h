...
#include <algorithm>
#include <assert.h>
//My own dynamic array class following AIE tutorials
template<class T>
class DynArray
{
public:
...
{
	assert((int)index < m_used && "Out of range");
...
void DynArray<T>::add(T item, int index)
{
	...
	assert(index >= 0 && index <= m_used && "Index not in range");
	if (m_used < m_size)
	{
